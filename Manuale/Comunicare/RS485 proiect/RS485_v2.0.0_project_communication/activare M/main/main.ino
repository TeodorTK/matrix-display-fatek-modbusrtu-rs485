...
// Adresa coil-ului M0 (0x7D1)
const uint16_t coilAddress = 0x7D;
...
void handleData(ModbusMessage response, uint32_t token) {
  ...
  if (response.getError() == SUCCESS) {
    ...
    if (response.getFunctionCode() == 0x05) {
      Serial.println(">>> Coil M0 scris cu succes!");
    }
  } else {
...
void setup() {
  ...
  Serial.print("  Coil Address M0: 0x");
  ...
  Serial.println("  2. Adresa coil-ului M0 (acum: 0x" + String(coilAddress, HEX) + ")");
  ...
  Serial.println("Începem să activăm/dezactivăm coil-ul M0 (0x7D1)...\n");
