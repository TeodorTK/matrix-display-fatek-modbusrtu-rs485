...
// Adresa primului coil (0x0)
const uint16_t coilAddress = 0x00001;
...
void handleData(ModbusMessage response, uint32_t token) {
  ...
  if (response.getError() == SUCCESS) {
    ...
    if (response.getFunctionCode() == 0x05) {
      Serial.println(">>> Coil scris cu succes!");
    }
  } else {
...
void setup() {
  ...
  Serial.println("  2. Adresa coil-ului (acum: 0x" + String(coilAddress, HEX) + ")");
  ...
  Serial.println("Începem să activăm/dezactivăm coil-ul 0x00000...\n");
