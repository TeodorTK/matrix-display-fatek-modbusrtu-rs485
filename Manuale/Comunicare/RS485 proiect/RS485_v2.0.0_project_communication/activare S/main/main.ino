...
// Adresa coil-ului S0 (0x1770)
const uint16_t coilAddress = 0x1770;
...
void handleData(ModbusMessage response, uint32_t token) {
  ...
  if (response.getError() == SUCCESS) {
    ...
    if (response.getFunctionCode() == 0x05) {
      Serial.println(">>> Coil S0 scris cu succes!");
    }
  } else {
...
void setup() {
  ...
  Serial.print("  Coil Address S0: 0x");
  ...
  Serial.println("  2. Adresa coil-ului S0 (acum: 0x" + String(coilAddress, HEX) + ")");
  ...
  Serial.println("Începem să activăm/dezactivăm coil-ul S0 (0x1770)...\n");
